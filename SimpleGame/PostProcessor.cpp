...
{
    blurProgram_ = LoadShaderProgram(L"Fullscreen.vs", L"Blur.fs");
    compositeProgram_ = LoadShaderProgram(L"Fullscreen.vs", L"Composite.fs");
    if (!blurProgram_ || !compositeProgram_)
    {
        Shutdown();
        return false;
    }
...
{
    if (width == width_ && height == height_ && scene_.fbo)
    {
        return true;
    }
    Release(scene_);
    ...
    for (auto& t : soft_)
    {
        ok = Allocate(t, halfWidth_, halfHeight_) && ok;
    }
    for (auto& t : bloom_)
    {
        ok = Allocate(t, halfWidth_, halfHeight_) && ok;
...
{
    glBindFramebuffer(GL_FRAMEBUFFER, target.fbo);
    glViewport(0, 0, halfWidth_, halfHeight_);
    glUseProgram(blurProgram_);
    glUniform1i(blurSource_, 0);
    glUniform2f(blurStep_, dx, dy);
    glUniform1i(blurExtract_, extract);
    glActiveTexture(GL_TEXTURE0);
...
void PostProcessor::Composite()
{
    ...
    const GLuint textures[] = {
        scene_.texture,
        settings.enabled && settings.edgeBlur ? soft_[1].texture : scene_.texture,
    ...
    glUniform1i(sceneUniform_, 0);
    glUniform1i(softUniform_, 1);
    glUniform1i(bloomUniform_, 2);
    glUniform1i(enabledUniform_, settings.enabled);
    glUniform1f(exposureUniform_, settings.exposure);
    glUniform3f(amountsUniform_,
                settings.bloom ? settings.bloomStrength : 0,
...
bool PostProcessor::VerifyHDR() const
{
    ...
    glBindFramebuffer(GL_READ_FRAMEBUFFER, scene_.fbo);
    ...
    for (size_t i = 0; i < pixels.size(); i += 4)
    {
        for (int c = 0; c < 3; ++c)
        {
            finite = finite && std::isfinite(pixels[i + c]);
            peak = std::max(peak, pixels[i + c]);
        }
    }
...
void PostProcessor::Shutdown()
{
    ...
    blurProgram_ = compositeProgram_ = vao_ = 0;
    width_ = height_ = 0;
}
